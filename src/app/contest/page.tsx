...
// entry showing zero, which reads as a head start nobody can catch up on — and
// it is meaningless anyway, since the contest is decided by votes.
//
// So the contest surfaces count only what a film earned AFTER it entered.
// Every entrant therefore starts from zero on the day they join, whether the
// film is new or three months old. Lifetime counts are untouched everywhere
// else — the film page, district feeds and rails all still show real totals.
async function withEntryScopedStats<T extends { film_id: string; created_at: string }>(
  entries: T[],
) {
